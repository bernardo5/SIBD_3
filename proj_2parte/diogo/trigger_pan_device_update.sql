...
if(
	exists(SELECT start, end FROM Connects
		WHERE(
			...
			and (((new.snum != Connects.snum) and (new.manuf != Connects.manuf)) and (new.pan = Connects.pan))
			or (((new.snum = Connects.snum) and (new.manuf = Connects.manuf)) and (new.pan != Connects.pan))	
		)
	)
)
	then	
